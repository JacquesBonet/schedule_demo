...
const mapDirection = (direction: Axis, reverse = false): Axis => {
...
export const mapFlexProps = <T extends BoxProps | GridProps>(props: BaseFlexProps<T>): FlexReturnType<T> => {
  const {x, y, row, column, flexDirection, reverse, nowrap, height, width, backgroundColor, position,top, left, p, pb, pl, pr, pt, px, m, mb, ml, mr, mt} = props;
